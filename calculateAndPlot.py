...
def main(ra1cat=("ge4j","eq0b"),jet=None):
	...
	for modAndPdf in c.mods_and_pdfs :
		...
		for pdfSet in modAndPdf[1] :
			...
			numFile = r.TFile("%s/%sJet_pn_%s.root"%(c.version,jet,model),"READ")
			if "MSTW" in pdfSet:
				numHist = numFile.Get("eff_%s_mstw_%s" % (model,"%s_%s"%ra1cat) if ra1cat else "eff_%s_mstw"%(model))
			else:
				numHist = numFile.Get("eff_%s_%s" % (model,"%s_%s"%ra1cat) if ra1cat else "eff_"+model)
				
			denFile = r.TFile("normalized_acc_output/%s/%sJet_normalized.root"%(c.version,jet),"READ")
			denHist = []
			for i in range(c.nPdfDict[pdfSet]) : 
				denHist.append(denFile.Get("eff_%s_%s_%s_%s"%(model,"%s_%s"%ra1cat,pdfSet,i) if ra1cat else "eff_%s_%s_%s"%(model,pdfSet,i)))
			...
			for xbin in range(denHist[0].GetXaxis().GetNbins()) :
				for ybin in range(denHist[0].GetYaxis().GetNbins()) :
					if (result.GetBinContent(xbin+1,ybin+1) == 0.0) :
						continue #LL ?
					if (denHist[0].GetBinContent(xbin+1,ybin+1) == 0.0) : 
						continue #LL ?
